        ...
        @if($article)
            <!-- post featured & title -->
                ...
                <div class="meta group">
                    ...
                    <p class="comments"><span><a href="#comments" title="Comment on This is the title of the first article. Enjoy it.">{{ count($article->comments)>0 ? count($article->comments) : '0' }}
                    ...
                    <ol class="commentlist group">
                        @foreach($com as $k => $item)
                            @if($k !== 0)
                                @break
                            @endif
                            @include(env('THEME') . '.comment', ['items' => $item])
                        @endforeach
                    </ol>
                    @endif
                </div>
                ...
                <div id="respond">
                    ...
                    <form action="{{ route('comment.store') }}" method="post" id="commentform">
                        <!-- Этот код csrf_field() обязательный -->
                        {{ csrf_field() }}
